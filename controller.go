...
func (ctrl *ProvisionController) enqueueClaim(obj interface{}) {
	...
	glog.Infof("enqueueClaim uid: %v", uid)
...
func (ctrl *ProvisionController) syncClaim(obj interface{}) error {
	...
	if !should {
		logrus.Infof("claim(%s/%s(%v)) should not provisioner, claim.Spec.VolumeName=%s",claim.Namespace, claim.Name, claim.UID, claim.Spec.VolumeName)
	} else {
		logrus.Infof("claim(%s/%s(%v)) starts to provisioner, claim.Spec.VolumeName=%s",claim.Namespace, claim.Name, claim.UID, claim.Spec.VolumeName)
	}
...
func (ctrl *ProvisionController) shouldProvision(claim *v1.PersistentVolumeClaim) (bool, error) {
	...
	if ctrl.kubeVersion.AtLeast(utilversion.MustParseSemantic("v1.5.0")) {
		glog.Infof("comes to ctrl.kubeVersion.AtLeast(utilversion.MustParseSemantic('v1.5.0'))")
		if provisioner, found := claim.Annotations[annStorageProvisioner]; found {
			if ctrl.knownProvisioner(provisioner) {
				glog.Infof("return from knownProvisioner knownProvisioner: %v", provisioner)
				return true, nil
			}
		}
	} else {
		glog.Infof("kubernetes version less than v1.5.0")
		...
		if class.Provisioner != ctrl.provisionerName {
			glog.Infof("return from class.Provisioner(%v) != ctrl.provisionerName(%v) knownProvisioner: %v", class.Provisioner, ctrl.provisionerName)
			return false, nil
		}

		glog.Infof("return from (kubernetes version less than v1.5.0)")

		return true, nil
	}

	glog.Infof("return from the end!")
	return false, nil
}
...
func (ctrl *ProvisionController) canProvision(claim *v1.PersistentVolumeClaim) error {
	...
	if CheckPersistentVolumeClaimModeBlock(claim) && !ctrl.supportsBlock() {
...
func (ctrl *ProvisionController) updateProvisionStats(claim *v1.PersistentVolumeClaim, err error, startTime time.Time) {
	...
	if err != nil {
		PersistentVolumeClaimProvisionFailedTotal.WithLabelValues(class).Inc()
	} else {
		PersistentVolumeClaimProvisionDurationSeconds.WithLabelValues(class).Observe(time.Since(startTime).Seconds())
		PersistentVolumeClaimProvisionTotal.WithLabelValues(class).Inc()
	}
}
...
func (ctrl *ProvisionController) updateDeleteStats(volume *v1.PersistentVolume, err error, startTime time.Time) {
	...
	if err != nil {
		PersistentVolumeDeleteFailedTotal.WithLabelValues(class).Inc()
	} else {
		PersistentVolumeDeleteDurationSeconds.WithLabelValues(class).Observe(time.Since(startTime).Seconds())
		PersistentVolumeDeleteTotal.WithLabelValues(class).Inc()
	}
}
